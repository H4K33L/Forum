	...
	if r.Method == "POST" {
		...
		if err != nil {
			if err == http.ErrNoCookie {
				log.Fatal("postmodification PostSupr, cookie not found Uuid :", err)
			}
			log.Fatal("postmodification PostSupr, Error retrieving cookie Uuid :", err)
		}
		...
		if ID != "" && post.Uuid == Uuid.Value {
			...
			if err != nil {
				log.Fatal("postmodification PostSupr, err deleting post :", err)
			}
...
func PostEdit(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		if err != nil {
			if err == http.ErrNoCookie {
				log.Fatal("postmodification PostEdit, cookie not found Uuid :", err)
			}
			log.Fatal("postmodification PostEdit, Error retrieving cookie Uuid :", err)
		}
		...
		if ID != "" && post.Uuid == Uuid.Value {
			...
			if err != nil {
				log.Fatal("postmodification PostEdit, err Editing post :", err)
			}
		}
	}
}
